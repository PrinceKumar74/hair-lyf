...
import { useNavigate } from 'react-router-dom';
import EmptyWishlistImg from '/womanWithCart/emptyCart.png';

const Wishlist = () => {
  // Updated selector to access the items array inside wishlist state
          ...
          {wishlistItems.map((item) => {
            ...
            return (
              ...
              >
                <div className="relative w-full h-64 mb-3 rounded-md flex items-center justify-center overflow-hidden">
                  <img
                    src={item.image || 'https://placehold.co/300x400/f0f0f0/cccccc?text=No+Image'}
                    alt={item.name || 'Product Image'}
                    className="max-w-full max-h-full object-contain"
                    onError={(e) => e.target.src = 'https://placehold.co/300x400/f0f0f0/cccccc?text=Image+Error'}
                  />
                  <button
                    onClick={() => dispatch(removeFromWishlist(item.id))}
                    className="absolute top-2 right-2 p-1 bg-white rounded-full text-gray-400 hover:text-red-500 hover:bg-gray-100 transition duration-150 shadow"
